...
def fetch_question_data(url):
    try:
        ...
        # Method 2: Community Vote (Most reliable often)
        vote_notes = ""
        tally_script = soup.select_one('.voted-answers-tally script')
        if tally_script:
            try:
                vote_data = json.loads(tally_script.string)
                # vote_data is a list of dicts, e.g. [{"voted_answers": "D", "vote_count": 62, "is_most_voted": true}]
                for vote in vote_data:
                    if vote.get('is_most_voted'):
                        vote_notes = f"Community Voted: {vote.get('voted_answers')} ({vote.get('vote_count')} votes)"
                        # Optionally override suggested answer if community differs? 
                        # For now, we list both.
